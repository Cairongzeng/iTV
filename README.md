...
services:
  itv-core:
    image: ghcr.io/cairongzeng/itv:latest
    container_name: itv-core
    restart: on-failure
    ports:
      - '3000:3000'
    environment:
      ...
      - KVROCKS_URL=redis://itv-kvrocks:6666
    networks:
      - itv-network
    depends_on:
      - itv-kvrocks
...
services:
  itv-core:
    ...
    environment:
      ...
      - REDIS_URL=redis://itv-redis:6379
    networks:
      - itv-network
    depends_on:
      - itv-redis
...
2. **添加 iTV 服务**
   - 点击 "Add Service" > "Docker Images"
   - 输入镜像名称：`ghcr.io/cairongzeng/itv:latest`
   - 配置端口：`3000` (HTTP)
...
3. **配置环境变量**
   ...
   NEXT_PUBLIC_SITE_NAME=iTV Enhanced
   ANNOUNCEMENT=欢迎使用 iTV Enhanced Edition
...
#### 5. 设置访问域名（必须）
   ...
   - 在 iTV 服务页面，点击 "Networking" 或 "网络" 标签
...
完成部署后为空壳应用，无播放源，需要站长在管理后台的配置文件设置中填写配置文件（支持订阅）
...
iTV 支持标准的苹果 CMS V10 API 格式。

## 订阅

将完整的配置文件 base58 编码后提供 http 服务即为订阅链接。
...
| NEXT_PUBLIC_SITE_NAME               | 站点名称                                     | 任意字符串                       | iTV                                                                                                                     |
...
为了您的安全和避免潜在的法律风险，在部署时**强烈建议关闭公网注册**：
...
[![Star History Chart](https://api.star-history.com/svg?repos=Cairongzeng/iTV&type=Date)](https://www.star-history.com/#Cairongzeng/iTV&Date)
