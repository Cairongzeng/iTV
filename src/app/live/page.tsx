...
function LivePageClient() {
    ...
    constructor(config: any) {
      ...
      this.load = function (context: any, config: any, callbacks: any) {
        ...
        try {
          ...
          url.searchParams.set('itv-source', currentSourceRef.current?.key || '');
  ...
  useEffect(() => {
    const preload = async () => {
      ...
      // precheck type
      let type = 'm3u8';
      const precheckUrl = `/api/live/precheck?url=${encodeURIComponent(videoUrl)}&itv-source=${currentSourceRef.current?.key || ''}`;
      ...
      const targetUrl = `/api/proxy/m3u8?url=${encodeURIComponent(videoUrl)}&itv-source=${currentSourceRef.current?.key || ''}`;
      try {
        // 创建新的播放器实例
